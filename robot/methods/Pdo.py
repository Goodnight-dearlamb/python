...
class Pdo:
    ...
    def __init__(self, host="127.0.0.1", user="root", password="", database="demo", charset="utf8", port=3306):
        try:
            self.connect = mysql.connect(host=host, user=user, password=password, database=database, charset=charset)
    ...
    # 查询单条数据 返回dist
    def find(self, table, where='', field='*'):
        data = {}
...
if __name__ == "__main__":
    ...
    # print(pdo.count("kuaixun", "id=1"))
    # print(pdo.find("kuaixun", "id=1", "id"))
    # print(pdo.select("kuaixun", "id<10", "origin_id"))
    # pdo.save("kuaixun", "id = 1", {'name': 'asd', 'title': 'qwe'})
